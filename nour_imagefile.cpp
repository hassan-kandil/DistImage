...
bool newimg (string img, string owner, int views){
    ...
    bool flag= true; // if there is a duplicate of the image name and owner name, change the flag to false.
...
void updatefile(fstream & imgfile){
    ...
    for (auto const &x:sharedimgs){
        imgfile<<x.first<<" "<<x.second.ownername<<" "<<x.second.viewsnum<<endl;
    }
}
