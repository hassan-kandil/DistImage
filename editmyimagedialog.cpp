...
#include <QString>
#include <string>
editmyimageDialog::editmyimageDialog(QWidget *parent, Peer *peer, QString imagename) :
    QDialog(parent),
    ui(new Ui::editmyimageDialog), peer(peer), imagename(imagename)
{
    ui->setupUi(this);
    ...
    for(int i=0; i<selectedViewers.size(); i++){
        ui->listWidget->addItem(QString::fromStdString("Viewer: " + selectedViewers[i].first + ". Views Left: " + std::to_string(selectedViewers[i].second)));

    }

}

editmyimageDialog::~editmyimageDialog()
{
    delete ui;
}
