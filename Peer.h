...
//  UDPSocketServer *sv;
  ...
  void getRequest() {
    ...
    if ((r = recvfrom(sv->s, Serverbuffer, BUFFER_SIZE, 0, (struct sockaddr *)&tempSocketAddress, &tempAddrlen)) < 0)
    {
    ...
    inet_ntop(AF_INET, &(tempSocketAddress.sin_addr), sender_ip,INET_ADDRSTRLEN);
...
//    size_t msgsize = requestMsg.getMessageSize();
    ...
    case 2002: // view request
    {
      ...
      Message view_request_reply(Reply,msg_char,strlen(msg_char),op_id_request,rpcid_request,0,0,0);
      ...
      if (sendto(sv->s, marshalled_message_reply,strlen((const char *)marshalled_message_reply), 0,(struct sockaddr *)&tempSocketAddress, tempAddrlen) < 0) {
    ...
    case 2003: // view approved
    {
      ...
      Message login_request_reply(Reply,msg_char,strlen(msg_char),op_id_request,rpcid_request,0,0,0);
      ...
      if (sendto(sv->s, marshalled_message_reply, strlen(marshalled_message_reply), 0, (struct sockaddr *)&tempSocketAddress,
                 tempAddrlen) < 0) {
    ...
    //   char * imfirstfrag = requestMsg.getMessage();
      ...
      Message image_reply(Reply,msg_char,strlen(msg_char),op_id_request,rpcid_request,fragd,fragc,MF);
      ...
      if (sendto(sv->s, marshalled_message_reply, strlen(marshalled_message_reply), 0, (struct sockaddr *)&tempSocketAddress, tempAddrlen) < 0) {
        perror("send to reply failed");
      }
...
//      }
       ...
       std::ofstream os("/home/hkandil/Downloads/distributed_pro-master-latest/new.jpeg");
       ...
       memset(little_buffer, 0, sizeof(little_buffer));
         ...
         while(MF){
          ...
          // string newY(imfrag,imfraglen);
           ...
           Message image_reply(Reply,msg_char,strlen(msg_char),op_id_new,rpcid_new,fragd,fragc_new,MF);
  ...
  void sendReply(char * temp_buffer) {
    ...
    struct sockaddr_in recievedAddr;
    socklen_t addresslength = sizeof(recievedAddr);
    if (sendto(sv->s, temp_buffer, strlen(temp_buffer), 0, (struct sockaddr *)&recievedAddr,
               addresslength) < 0) {
  ...
  int sign_up(string username, string password) {
    ...
    if (!nospecial) { // special chars
      ...
      Message sign_up_request(Request,msg_char,strlen (msg_char),1001,++requestID,0,0,0);
  ...
  int login(string username, string password) {
    ...
    if (!nospecial) { // special chars
      ...
      Message login_request(Request,msg_char,strlen(msg_char),1002,++requestID,0,0,0);
  ...
  int logout() {
    ...
    Message logout_request(Request,msg_char,strlen (msg_char),1003,++requestID,0,0,0);
 ...
 /*   struct pollfd ss;
  ...
  int upload(string pathname, string imagename, string defaultimage) {
    ...
    if (!nospecial) { // special chars
      ...
      Message upload_request(Request,msg_char,strlen(msg_char),2001,++requestID,0,0,0);
...
//      } else {
  ...
  void request_image(string selectedUser, string selectedImage, string path_full) {
    ...
    Message image_request(Request,msg_char,strlen (msg_char),2002,++requestID,0,0,0);
  //  char * marshalled_message = image_request.marshal();
    ...
    if ((n = sendto(sc->s, marshalled_message, strlen((const char *)marshalled_message), 0, (struct sockaddr *)&ownerSocket, sizeof(struct sockaddr_in))) < 0)
      perror("Send failed\n");

    memset(little_buffer, 0, sizeof(little_buffer));

    //Preparing a temp socket address to store the address of the receiver sending the reply in
    struct sockaddr_in tempSocketAddress;
    socklen_t tempAddrlen = sizeof(tempSocketAddress);

    if((r = recvfrom(sc->s, little_buffer, sizeof(little_buffer), 0, (struct sockaddr *) &tempSocketAddress,  &tempAddrlen))<0)
        perror("Receive Failed") ;
  ...
  void approve_image(string selectedUser, string selectedImage) {
    ...
    map<string, vector<string>> users;  //map containing the table of users (username, images names, socket addresses)
    users = this->getUsers();           //retreiving the map from the dos
    vector<string> images;
    images = users[selectedUser];  //Saving the vector of the selected user image names & socket addresses inside the variable images

    struct sockaddr_in receiverSocket; //instantiating the receiver socket address (the user who has requested my image)

    string remote_peer_ip = images[1];   //retriveing the ip of the receiver
    int remote_peer_port = std::stoi(images[2], nullptr, 0); // Refaay //Retreiving the port no of the receiver
    cout << "Remote IP " << remote_peer_ip << ", port " << remote_peer_port << endl;
    ...
    Message sendimage_request(Request,msg_char,strlen (msg_char),2003,++requestID,0,0,0);
        ...
        is.close();
                ...
                while (current_length_before_marshalling < length){
                    ...
                    if (current_length_before_marshalling + 10000 < length){
                        ...
                        Message imageFrag(Request,(newbuffer+current_length_before_marshalling),10000,2004,requestID,fragd,++fragc,MF);
                        ...
                        //  cout<<message<<endl;
                          if( (n = sendto(newSock->s, message, strlen(message), 0, (struct sockaddr *) &receiverSocket, sizeof(struct sockaddr_in))) < 0)
                          perror("Send failed\n");
                          cout<<"Current Sent: " << strlen(message)<<endl;
                          printf("Current Sent Total = %d.\n", current_length);

                          memset(little_buffer, 0, sizeof(little_buffer));
                          if((r = recvfrom(newSock->s, little_buffer, sizeof(little_buffer), 0, (struct sockaddr *) &tempSocketAddress,  &tempAddrlen))<0)
                              perror("Receive Failed") ;

                          cout<<"received length before unmarshalling: "<<little_buffer<<endl;

                          //unmarshal the reply
                          Message reply_received_length(reinterpret_cast<char*>(little_buffer));

                          string msg_received_length = reply_received_length.getUnmarshalledMessage();
                        ...
                        //checks if the total length of the packets sent is equal to the received length in the reply

                          if(received_length != current_length){
                              //if not reset the current_length_before_marshalling in order to resend the packet
                        ...
                        MF = 0;
                        Message imageLastFrag(Request,(newbuffer+current_length_before_marshalling),fakka,2004,requestID,fragd,++fragc,MF);
                        ...
                        //  cout<<message<<endl;
                          if( (n = sendto(sc->s, message_fakka, strlen(message_fakka), 0, (struct sockaddr *) &receiverSocket, sizeof(struct sockaddr_in))) < 0)
                          perror("Send failed\n");
  ...
  map<string, vector<string>> getUsers() {
    ...
    msg_char[0] =' ';
    Message view_request(Request,msg_char, strlen (msg_char),1100,++requestID,0,0,0);
