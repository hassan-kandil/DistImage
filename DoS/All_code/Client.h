...
class Client{
	...
	bool DoOperation(char *machine, int port, char message[]){
		...
		while(received_length != strlen(message)){ // != strlen(message)
			...
			if( (n = sendto(sc->s, little_buffer, strlen((const char*)little_buffer), 0, (struct sockaddr *) &yourSocketAddress, sizeof(struct sockaddr_in))) < 0)
		    perror("Send failed\n");

			if((r = recvfrom(sc->s, little_buffer, 10, 0, (struct sockaddr *) &peerSocketAddress,  &peerAddrlen))<0)
		...
		while (current_length < strlen(message)){ //  && r == 1 && little_buffer[0] == '1'
			...
			if( (n = sendto(sc->s, char_array, strlen(char_array), 0, (struct sockaddr *) &yourSocketAddress, sizeof(struct sockaddr_in))) < 0)
		    perror("Send failed\n");
