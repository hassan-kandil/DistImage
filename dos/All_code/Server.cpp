...
void Server::getRequest(){
  ...
  r = recvfrom(sv->s, little_buffer, LITTLE_BUFFER_SIZE, 0, (struct sockaddr *)&recievedAddr, &addresslength);
  ...
  if (sendto(sv->s, little_buffer, strlen((const char*)little_buffer), 0, (struct sockaddr *)&recievedAddr, addresslength) < 0) {
  ...
  while(current_received < received_length){
    ...
    if (sendto(sv->s, little_buffer, LITTLE_BUFFER_SIZE, 0, (struct sockaddr *)&recievedAddr, addresslength) < 0) {
...
void Server::sendReply(){
	if (sendto(sv->s, buffer, r, 0, (struct sockaddr *)&recievedAddr, addresslength) < 0) {
        perror("sendto failed");
  }
}

bool Server::serveRequests(){
	getRequest();
}
